...
def plot_damaged_cluster(index, true_results, all_random_results, metadata, evaluation,
    ...
    (pd.Series(evaluation).
     plot(color='g',
          legend=False, ax=ax))

    ax.set_xticks(range(11))
    ax.set_xticklabels(list('0123456789') + ['overall'])
...
def plot_all_damaged_clusters(true_results, random_results, metadata, evaluation,
                              alpha=0.1, figsize=(20, 15), ncols=3, title=None):
    ...
    for cluster_index, ax in zip(range(len(metadata)), axes_iter):
        plot_damaged_cluster(cluster_index, true_results, random_results, metadata, evaluation,
                             alpha=alpha, ax=ax)
