...
def extract_weights(model, with_bias=False):
...
def get_weights_paths(weight_directory):
...
def preprocess_dataset(dataset_path, hot_one=False):
    ...
    ds['X_train'] = (ds['X_train'] / 255).reshape(-1, 784)
    ds['X_test'] = (ds['X_test'] / 255).reshape(-1, 784)
